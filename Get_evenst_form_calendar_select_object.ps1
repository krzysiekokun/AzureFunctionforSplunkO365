...
$createdStartDate = Get-Date -Year 2023 -Month 1 -Day 01
...
foreach ($event in $eventsResponse.value) {
    ...
    @{Name='Do kiedy jest spotkanie zaplanowane'; Expression={
        if ($_.recurrence.range.endDate -eq "0001-01-01") {
            $_.recurrence.range.type
        } else {
            $_.recurrence.range.endDate
        }
    }},

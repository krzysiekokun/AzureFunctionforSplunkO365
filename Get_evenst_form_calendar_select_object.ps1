    ...
    foreach ($event in $eventsResponse.value) {
        ...
        if ($null -ne $event.recurrence -and $eventCreatedDate -ge $createdStartDate -and $eventCreatedDate -le $createdEndDate) {
            $selectedValues = $event | Select-Object -Property @{
                ...
                @{Name='Poczatek spotkania'; Expression={$_.start.dateTime}},
                @{Name='Koniec spotkania'; Expression={$_.end.dateTime}},
				@{Name='Anulowane?'; Expression={$_.isCancelled}},
                @{Name='Data utworzenia spotkania'; Expression={$_.createdDateTime}},
                @{Name='Data ostatniej modyfikacji spotkania'; Expression={$_.lastModifiedDateTime}},
